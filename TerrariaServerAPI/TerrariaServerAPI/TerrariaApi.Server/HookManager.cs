...
namespace TerrariaApi.Server
{
	...
	public class HookManager
	{
		...
		public Task AttachOTAPIHooks(string[] args)
		{
		...
		internal async Task<bool> InvokeNetGetData(byte msgId, MessageBuffer buffer, int index, int length)
		{
			if (Main.netMode == 2)
			{
						...
						using (var stream = new MemoryStream(buffer.readBuffer))
						{
							...
							using (var reader = new BinaryReader(stream))
							{
								...
								if (moduleId == Terraria.Net.NetManager.Instance.GetId<Terraria.GameContent.NetModules.NetTextModule>())
								{
									...
									if (await InvokeServerChat(buffer, buffer.whoAmI, @msg.Text, msg.CommandId))
									{
										return true;
									}
								}
		...
		internal Task<bool> InvokeServerChat(MessageBuffer buffer, int who, string text, Terraria.Chat.ChatCommandId commandId)
		{
