...
namespace TerrariaApi.Server.Hooking
{
	internal class NetHooks
	{
		...
		public static void AttachTo(HookManager hookManager)
		{
			...
			Hooks.MessageBuffer.GetData += async(x,y)=>await OnReceiveData(x,y);
		...
		static async Task OnReceiveData(object sender, Hooks.MessageBuffer.GetDataEventArgs e)
		{
			...
			else
			{
				...
				if (await _hookManager.InvokeNetGetData(msgId, e.Instance, readOffset, length))
				{
					e.Result = HookResult.Cancel;
				}
