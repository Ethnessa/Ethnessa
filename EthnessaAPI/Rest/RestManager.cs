...
namespace EthnessaAPI
{
	...
	public class RestManager
	{
		...
		private object ServerCommandV3(RestRequestArgs args)
		{
			...
			var restPlayerGroup = GroupManager.GetGroupByName(args.TokenData.UserGroupName);
		...
		{
			return new RestObject() { { "users", (UserAccountManager.GetUserAccounts()).Select(p => new Dictionary<string,object>(){
				{"name", p.Name},
				{"id", p.AccountId},
				{"group", p.Group},
			}) } };
		...
		private object UserUpdateV2(RestRequestArgs args)
		{
			...
			UserAccount account = UserAccountManager.GetUserAccountByName(args.Parameters["user"]);
			var response = new RestObject();
			if (!string.IsNullOrWhiteSpace(password))
			{
				try
				{
					UserAccountManager.SetUserAccountPassword(account, password);
			...
			if (!string.IsNullOrWhiteSpace(group))
			{
				try
				{
					UserAccountManager.SetUserGroup(account, group);
		...
		{
			var user = UserAccountManager.GetUserAccountByName(args.Parameters["user"]);

			try
			{
				UserAccountManager.RemoveUserAccount(user);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object BanDestroyV3(RestRequestArgs args)
		{
			...
			if (BanManager.RemoveBan(ticketNumber.ToString()))
			{
		...
		private object BanInfoV3(RestRequestArgs args)
		{
			...
			Ban ban = BanManager.GetBanById(ticketNumber);
		...
		private object GroupList(RestRequestArgs args)
		{
			...
			foreach (Group group in GroupManager.GetGroups())
			{
		...
		private object GroupInfo(RestRequestArgs args)
		{
			...
			Group group = GroupManager.GetGroupByName(args.Parameters["group"]);
			return new RestObject() {
				{"name", group.Name},
		...
		private object GroupDestroy(RestRequestArgs args)
		{
			...
			try
			{
				GroupManager.DeleteGroup(group.Name, true);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object GroupCreate(RestRequestArgs args)
		{
			...
			try
			{
				...
				GroupManager.AddGroup(name, args.Parameters["parent"], permList, args.Parameters["chatcolor"]);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object GroupUpdate(RestRequestArgs args)
		{
			...
			try
			{
				GroupManager.UpdateGroup(group.Name, parent, permList, chatcolor, group.Suffix, group.Prefix);
			}
			catch (Exception e)
			{
				return RestError(e.Message);
		...
		private object UserFind(EscapedParameterCollection parameters)
		{
			...
			try
			{
				switch (type)
				{
					...
					case "name":
						type = "name";
						account = UserAccountManager.GetUserAccountByName(name);
						break;
					case "id":
						account = UserAccountManager.GetUserAccountById(Convert.ToInt32(name));
						break;
					default:
		...
		private object GroupFind(EscapedParameterCollection parameters)
		{
			...
			var group = GroupManager.GetGroupByName(name);
			if (null == group)
