...
namespace EthnessaAPI.Handlers
{
	...
	/// </summary>
	public class LandGolfBallInCupHandler : IPacketHandler<GetDataHandlers.LandGolfBallInCupEventArgs>
	{
		/// <summary>
		...
		public async void OnReceive(object sender, GetDataHandlers.LandGolfBallInCupEventArgs args)
		{
			if (args.PlayerIndex != args.Player.Index)
			{
				ServerBase.Log.ConsoleDebug(GetString($"LandGolfBallInCupHandler: Packet rejected for AccountId spoofing. Expected {args.Player.Index}, received {args.PlayerIndex} from {args.Player.Name}."));
				args.Handled = true;
				return;
			}
			...
			{
				ServerBase.Log.ConsoleDebug(GetString($"LandGolfBallInCupHandler: X and Y position is out of world bounds! - From {args.Player.Name}"));
				args.Handled = true;
				return;
			}
			...
			{
				ServerBase.Log.ConsoleDebug(GetString($"LandGolfBallInCupHandler: Tile at packet position X:{args.TileX} Y:{args.TileY} is not a golf hole! - From {args.Player.Name}"));
				args.Handled = true;
				return;
			}
			...
			{
				ServerBase.Log.ConsoleDebug(GetString($"LandGolfBallInCupHandler: Invalid golf ball projectile AccountId {args.ProjectileType}! - From {args.Player.Name}"));
				args.Handled = true;
				return;
			}
			...
			{
				ServerBase.Log.ConsoleDebug(GetString($"GolfPacketHandler: Player did not have create a golf club projectile the last 5 seconds! - From {args.Player.Name}"));
				args.Handled = true;
				return;
			}
			...
			{
				ServerBase.Log.ConsoleDebug(GetString($"LandGolfBallInCupHandler: Item selected is not a golf club! - From {args.Player.Name}"));
				args.Handled = true;
				return;
			}
