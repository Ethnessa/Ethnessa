...
namespace EthnessaAPI
{
	...
	public class ServerPlayer
	{
		...
		public bool HasBuildPermission(int x, int y, bool shouldWarnPlayer = true)
		{
			...
			if (!ServerBase.Config.Settings.DisableBuild || HasPermission(Permissions.antibuild))
			{
				...
				if (!ServerBase.Config.Settings.SpawnProtection || HasPermission(Permissions.editspawn) || !Utils.IsInSpawn(x, y))
				{
					...
					if (RegionManager.CanBuild(x, y, this))
					{
						return true;
					}
				}
		...
		/// </summary>
		public List<GetDataHandlers.ProjectileStruct> RecentlyCreatedProjectiles = new List<GetDataHandlers.ProjectileStruct>();
		...
		public bool SaveServerCharacter()
		{
			...
			try
			{
				...
				CharacterManager.InsertPlayerData(this);
				return true;
			}
			catch (Exception e)
			{
		...
		public bool Kick(string reason, bool force = false, bool silent = false, string? adminUserName = null, bool saveServerSideInventory = false)
		{
			if (!ConnectionAlive)
			{
				return true;
		...
		public bool Ban(string reason, string adminUserName = "Server")
		{
			...
			All.SendInfoMessage(string.IsNullOrWhiteSpace(adminUserName)
