...
namespace TShockAPI.ServerCommands
{
	public class AccountInfoCommand : Command
	{
		...
		public override List<string> Permissions { get; protected set; } = new() { TShockAPI.Permissions.checkaccountinfo };
		...
		private static void ViewAccountInfo(CommandArgs args)
		{
			...
			if (!string.IsNullOrWhiteSpace(username))
			{
				...
				if (account != null)
				{
					...
					if (args.Player.Group.HasPermission(TShockAPI.Permissions.advaccountinfo))
					{
