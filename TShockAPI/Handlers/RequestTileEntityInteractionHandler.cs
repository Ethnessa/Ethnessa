...
namespace TShockAPI.Handlers
{
	...
	{
		public async Task OnReceive(RequestTileEntityInteractionEventArgs args)
		{
			if (args.TileEntity is TEHatRack && !await args.Player.HasBuildPermissionForTileObject(args.TileEntity.Position.X, args.TileEntity.Position.Y, TEHatRack.entityTileWidth, TEHatRack.entityTileHeight, false))
			{
				args.Player.SendErrorMessage(GetString("You do not have permission to modify a Hat Rack in a protected area!"));
				args.Handled = true;
				return;
			}
			else if (args.TileEntity is TEDisplayDoll && !await args.Player.HasBuildPermissionForTileObject(args.TileEntity.Position.X, args.TileEntity.Position.Y, TEDisplayDoll.entityTileWidth, TEDisplayDoll.entityTileHeight, false))
			{
				args.Player.SendErrorMessage(GetString("You do not have permission to modify a Mannequin in a protected area!"));
				args.Handled = true;
				return;
			}
			else if (!await args.Player.HasBuildPermission(args.TileEntity.Position.X, args.TileEntity.Position.Y, false))
			{
